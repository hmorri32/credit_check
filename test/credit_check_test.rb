...
class UnitTests < Minitest::Test
  ...
  def test_card_existence
    ...
    assert_instance_of CreditMethods, @methods
  end

  def test_splitter
    split = @methods.splitter("4929735477250543")
    assert_equal ["4", "9", "2", "9", "7", "3", "5", "4", "7", "7", "2", "5", "0", "5", "4", "3"], split
  end

  def test_integers
    ints = @methods.integers(["1", "2", "3"])
    assert_equal [1,2,3], ints 
  end

  def test_reverse_method
    reversed = @methods.reverse_it([1, 2, 3])
    assert_equal [3, 2, 1], reversed
  end

  def test_every_other_doubler
    doubled = @methods.every_other_times_two([1, 2, 3, 4, 5])
    assert_equal [1, 4, 3, 8, 5], doubled
  end

  def test_add_over_ten
    summed = @methods.add_over_ten([12, 2, 13])
    assert_equal [3, 2, 4], summed
  end

  def test_add_all_digits 
    sum = @methods.add_all_digits([1, 2, 3, 4, 5, 6])
    assert_equal 21, sum 
  end
end
